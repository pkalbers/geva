...
inline std::string load_hap(const std::string & hap_file, const std::string & sample_file, const Gen::Map & gmap, const std::string & outfile,
...
{
	std::cout << "Loading haplotype from HAP + SAMPLE files to binary file" << std::endl;
	std::clog << "Loading haplotype from HAP + SAMPLE files to binary file" << std::endl;
	
	std::cout << "<< " << hap_file << std::endl;
	std::clog << "<< " << hap_file << std::endl;
	...
	{
		Clock   time;
		LoadHap load(hap_file, sample_file); // reads sample vector
