...
#ifndef ibd_by_pair_h
#define ibd_by_pair_h
...
inline IBD::Pair::Result::Data ibd_by_pair(const IBD::DetectMethod method, const decimal_t max_miss, const std::string & output, const Gen::Share::Data share, const Gen::Grid::Data grid, Clock & time, const IBD::HMM::Model::Data hmm_model = nullptr, const size_t threads = 1)
{
	...
	switch (method)
	{
		...
		case IBD::DETECT_HMM:
			...
			break;
		case IBD::DETECT_SIM:
			throw std::logic_error("If you see this, your computer is broken!");
			break;
	...
	try
	{
		...
		IBD::Pair::Result::Data result = std::make_shared< IBD::Pair::Result >(share);
		...
		Progress prog(result->get().size(), "pairs");
		
		IBD::Pair::Target::List::const_iterator list, end = result->get().cend();
		
		if (threads > 1)
		{
			Threadpool< IBD::Pair::Detect > pool(threads - 1, &IBD::Pair::Detect::run);
			
			for (list = result->get().cbegin(); list != end; ++list)
			{
				pool.task(IBD::Pair::Detect(method, max_miss, *list, grid, hmm_model));
			}
		...
		else
		{
			for (list = result->get().cbegin(); list != end; ++list)
			{
				IBD::Pair::Detect detect(method, max_miss, *list, grid, hmm_model);
		...
		switch (method)
		{
			...
			case IBD::DETECT_SIM: throw std::logic_error("Why do you see this?"); break;
			case IBD::DETECT_VOID: return nullptr;
		}
