...
size_t Share::select(const std::set<size_t> & position, const Grid::Data grid, const bool is_id)
{
	if (this->good)
	{
		throw std::runtime_error("Sharing index already generated");
	...
	if (is_id)
	{
		for (pos = position.cbegin(); pos != end; ++pos)
		{
			...
			{
				if (marker->index.value == focus)
				{
					const size_t target = marker->hap_count[H1];
